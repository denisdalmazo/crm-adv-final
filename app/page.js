...
import { Users, Scale, Clock, CheckCircle, Plus, X, MessageSquare } from 'lucide-react';
...
export default function CRM() {
  ...
  const openWhatsApp = (lead) => {
    ...
    const message = encodeURIComponent(`Olá ${lead.nome}, aqui é da Dalmazo Advogados. Gostaria de falar sobre o seu caso.`);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-4 md:p-8 font-sans">
      <header className="flex justify-between items-center mb-8 border-b border-slate-700 pb-6 text-center md:text-left">
        <div>
          <h1 className="text-2xl font-bold text-blue-400">DALMAZO | CRM</h1>
          <p className="text-slate-500 text-[10px] uppercase font-black tracking-widest">Gestão Jurídica Ativa</p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-all shadow-lg shadow-blue-900/20"
        >
          <Plus size={18} /> Novo
        </button>
      </header>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-700">
              {loading ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-500 animate-pulse">Sincronizando...</td></tr>
              ) : leads.map((lead, i) => (
                <tr key={i} className="hover:bg-slate-700/30 transition-colors">
                  <td className="p-4 text-center">
                    <button 
                      onClick={() => openWhatsApp(lead)}
                      className="bg-green-500 hover:bg-green-400 text-slate-900 p-2 rounded-full inline-flex items-center justify-center shadow-lg shadow-green-900/20"
                    >
                  ...
                  <td className="p-4">
                    ...
                    <div className="text-[10px] text-slate-500 font-mono italic">{lead.telefone}</div>
                  </td>
                  <td className="p-4 text-xs text-slate-300 font-medium">{lead.tipo_acao}</td>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-slate-700 w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-red-400 transition-colors"><X /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <input required value={nome} onChange={e => setNome(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:border-blue-500 outline-none" placeholder="Nome do Cliente" />
              <div className="grid grid-cols-2 gap-4">
                <input value={telefone} onChange={e => setTelefone(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:border-blue-500 outline-none" placeholder="WhatsApp" />
              ...
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-black uppercase tracking-widest text-sm shadow-xl shadow-blue-900/30 transition-all">Salvar no CRM</button>
            </form>
          </div>
        </div>
      )}
...
function StatCard({ label, value, color }) {
  ...
  return (
    <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 text-center">
      <div className="text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">{label}</div>
